            ...
            for parent in current_generation:
                for _ in range(2):
                    ...
                    if affinity >= temp_aff_thresh and min_stop == 0:
                        ...
                        selection_p = 0.005 / min_stop
                    else:
                        selection_p = 0.001 / min_stop
...
if __name__ == "__main__":
    ...
    while(current_run <= n_runs):
        print(f"\n--- RUN {current_run} ---")
        run_folder = os.path.join(base_output_dir, f"run_{current_run}_gen{g_max}_minSeq{min_uniq_seq}")
        ...
        if stats['num_unique_sequences'] >= min_uniq_seq and stats['max_affinity'] >= t_max:
            ...
            keys_to_include = ['id', 'a_sequence', 'frame', 'cdr3_start', 'cdr3_end', 'cdr3_len', 'generation', 'parent', 'mutations', 'affinity', 'abundance']
